      ...
      if (authProvider.user != null) {
        setState(() {
          ...
          dob = authProvider.user!.dateOfBirth != null
              ? authProvider.user!.dateOfBirth!.toIso8601String().substring(0, 10)
              : '';
  ...
  Widget build(BuildContext context) {
    return Consumer<AuthProvider>(
      builder: (context, authProvider, child) {
        // Cập nhật email và name từ AuthProvider
        if (authProvider.user != null) {
          email = authProvider.user!.email;
          emailController.text = email;
          name = authProvider.user!.fullName ?? '';
          ...
          address = authProvider.user!.address ?? '';
          addressController.text = address;
          print('User data loaded: ${authProvider.user!.email}, ${authProvider.user!.fullName}');
        } else {
      ...
      return Row(
        children: [
          ...
          ElevatedButton(
            onPressed: () async {
              final success = await Provider.of<AuthProvider>(context, listen: false)
                  .updateProfile(dateOfBirth: dobController.text);
              if (success) {
                setState(() {
